...
const API = {
  ...
  qr: {
    ...
    cardAdmin:     (login, password) => apiFetch(`/qr/card/admin/${login}`, { method: "POST", body: JSON.stringify({ password }) }),
    login:         (code, password) => apiFetch("/qr/login",         { method:"POST", body:JSON.stringify({ code, password }) }),
